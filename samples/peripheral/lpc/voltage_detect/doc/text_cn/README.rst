...
Voltage Detection
==================
该示例验证 :term:`LPC` 电压检测功能。

使用 P2_2 作为电压检测的引脚。当检测 P2_2 的输入电压高于设定阈值时会触发 LPC 中断。
...
2. 当 P2_2 检测到输入电压高于 1080 mV 时，触发 :c:macro:`LPC_INT_VOLTAGE_DETECT` 中断，在 Debug Analyzer 工具内打印进入电压检测中断的 log 。
   ::
...
* 工程路径: :file:`sdk\\sample\\peripheral\\lpc\\voltage_detect\\proj`
* 源码路径: :file:`sdk\\sample\\peripheral\\lpc\\voltage_detect\\src`
...
4. 调用 :cpp:any:`LPC_INTConfig`，配置 LPC 比较中断 :c:macro:`LPC_INT_VOLTAGE_DETECT` 和 NVIC。 NVIC 相关配置可参考 :ref:`中断配置<general_nvic_config_cn>`。
5. 调用 :cpp:any:`LPC_Cmd` ，使能 LPC 比较。
...
1. 当 P2_2 检测到电压高于设置的阈值 1080 mV 时，触发 :c:macro:`LPC_INT_VOLTAGE_DETECT` 中断，在中断函数内打印相关信息，清除中断标志位。
