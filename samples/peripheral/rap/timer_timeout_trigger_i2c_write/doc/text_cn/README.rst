...
================================
该示例演示了如何通过 :term:`RAP` (Real Autonomous Peripheral) 机制，利用 TIMER 的超时事件触发 I2C Master 发起一次写传输 (Write Transaction)，无需 CPU 介入启动过程。

在本示例中，I2C Master 被配置为 **Wrapper 模式**。这种模式允许预先配置好传输类型（本例为纯写模式）和待发送的数据，当 RAP 事件（如 TIMER 超时）到达时，硬件自动执行完整的 I2C 写时序。

主要流程：
1. **预配置**: I2C Master 配置为 Wrapper 模式，设定为 Write 模式，并预填发送 FIFO。
2. **触发**: TIMER 每隔 1 秒产生超时事件，通过 RAP 触发 I2C Master 启动传输。
3. **传输**: I2C Master 自动发送 10 字节数据 (0, 1, ... 9)。
4. **处理**: CPU 仅在 Slave 收到数据满或检测到停止信号时，在中断中读取数据并设置标志位通知主函数，主函数中打印数据。
...
1. 可配置如下宏修改 I2C 参数。
   ...
   .. code-block:: c
    ...
    #define I2C_MASTER_WRITE_LEN        10      /* Master 写入 10 字节 */
...
* 工程路径: :file:`sdk\\sample\\peripheral\\rap\\timer_trigger_i2c_write\\proj`
* 源码路径: :file:`sdk\\sample\\peripheral\\rap\\timer_trigger_i2c_write\\src`

初始化
------
1. **I2C Slave 初始化**: 配置为从机模式，开启 :c:macro:`I2C_INT_RX_FULL` 和 :c:macro:`I2C_INT_STOP_DET` 中断。
2. **I2C Master 初始化**:
   ...
   * 调用 ``driver_i2c_master_wrapper_config`` 启用 **Wrapper 模式**：
     ...
     * :cpp:any:`I2C_WrapperSetTransMode`: 设置模式为 :cpp:any:`I2C_WRAPPER_TRANS_MODE_WRITE`。
     * 预填充 Write FIFO。
...
3. **主函数处理**:
   I2C Slave 接收到数据后，在中断函数内设置标志位 ``is_receive_done`` 通知主函数。主函数中收到通知后，打印接收到的数据长度和内容，并验证数据是否正确。
