 ...
 *
 * DFU:
 *   The DFU run-time instance is registered next to the HID interface, so the
 *   mouse advertises DFU_DETACH. On detach the HID device is torn down and a
 *   separate DFU mode device is brought up, exposing the two flash slots from
 *   src/dfu_slots.c as alternate settings. This is a one way switch, the mouse
 *   only comes back after a reboot.
 */
...
/*
 * DFU mode device, brought up only after the host requests DFU_DETACH. It uses
 * a different product ID than the run-time device so the host does not reuse
 * the cached HID configuration.
 */
USBD_DEVICE_DEFINE(dfu_usbd,
		   DEVICE_DT_GET(DT_NODELABEL(zephyr_udc0)),
		   CONFIG_SAMPLE_USBD_VID, CONFIG_APP_DFU_MODE_PID);
...
static const uint8_t dfu_attributes = (IS_ENABLED(CONFIG_SAMPLE_USBD_SELF_POWERED) ?
				       USB_SCD_SELF_POWERED : 0);

USBD_CONFIGURATION_DEFINE(dfu_fs_config, dfu_attributes,
			  CONFIG_SAMPLE_USBD_MAX_POWER, &dfu_fs_cfg_desc);
USBD_CONFIGURATION_DEFINE(dfu_hs_config, dfu_attributes,
			  CONFIG_SAMPLE_USBD_MAX_POWER, &dfu_hs_cfg_desc);
...
/* Tear down the run-time (HID) device and bring up the DFU mode device in its
 * place. Runs on the system workqueue from the DFU class detach handler.
 */
static void switch_to_dfu_mode(struct usbd_context *const ctx)
{
	int err;

	LOG_INF("Detach USB device, switching to DFU mode");
	usbd_disable(ctx);
	usbd_shutdown(ctx);
	...
	if (USBD_SUPPORTS_HIGH_SPEED &&
	    usbd_caps_speed(&dfu_usbd) == USBD_SPEED_HS) {
		...
		if (err) {
			LOG_ERR("Failed to register DFU mode class (HS)");
			return;
		}
	...
	if (err) {
		LOG_ERR("Failed to register DFU mode class (FS)");
		return;
	}
	...
	if (err) {
		LOG_ERR("Failed to initialize DFU mode device (%d)", err);
		return;
	}
	...
	if (err) {
		LOG_ERR("Failed to enable DFU mode device (%d)", err);
	}
}
