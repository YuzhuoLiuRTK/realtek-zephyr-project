...
{
	// DBG_DIRECT("[%s] dlps_check_flag%d line%d", __func__, dlps_check_flag, __LINE__);
	return dlps_check_flag;
}

#if !CONFIG_PM_DEVICE
static void pm_enter_pad_config(void)
{
	// Pad_Config(P3_3, PAD_SW_MODE, PAD_IS_PWRON, PAD_PULL_UP, PAD_OUT_DISABLE, PAD_OUT_HIGH);
	// System_WakeUpPinEnable(P3_3, PAD_WAKEUP_POL_LOW, PAD_WAKEUP_DISABLE);
}

static void pm_exit_pad_config(void)
{
	// Pad_Config(P3_3, PAD_PINMUX_MODE, PAD_IS_PWRON, PAD_PULL_UP, PAD_OUT_DISABLE,
	// PAD_OUT_HIGH); Pinmux_Config(P3_3, UART2_RX); System_WakeUpPinDisable(P3_3);
	// Pad_Config(P3_2, PAD_PINMUX_MODE, PAD_IS_PWRON, PAD_PULL_UP, PAD_OUT_DISABLE,
	// PAD_OUT_HIGH); Pinmux_Config(P3_2, UART2_TX);
}
...
int main(void)
{
	...
	// power_check_cb_register(app_check);
	DBG_DIRECT("[%s] line%d", __func__, __LINE__);
	platform_pm_register_callback_func_with_priority((void *)app_check, PLATFORM_PM_CHECK, 1);
	DBG_DIRECT("[%s] line%d", __func__, __LINE__);

#if !CONFIG_PM_DEVICE
	// io_dlps_register_enter_cb(app_store);
	// io_dlps_register_exit_cb(app_restore);
	// power_stage_cb_register(app_store, POWER_STAGE_STORE);
...
static int shell_pm_test_counter(const struct shell *sh, size_t argc, char **argv)
{
	...
	DBG_DIRECT("[%s] dlps_check_flag = 1", __func__);
	dlps_check_flag = PM_CHECK_PASS;
	// k_sem_take(&app_sem, K_MSEC(timeout_ms + 100));
	// k_sem_take(&app_sem, K_NO_WAIT);
	k_sem_init(&app_sem, 0, 1);
	k_sem_take(&app_sem, K_FOREVER);

	// k_sem_count_get(&top_cnt_sem);
	dlps_check_flag = PM_CHECK_FAIL;
	DBG_DIRECT("[%s] wakeup", __func__);
...
static int shell_pm_test_spi(const struct shell *sh, size_t argc, char **argv)
{
	...
	dlps_check_flag = PM_CHECK_PASS;
	k_sem_init(&app_sem, 0, 1);
	k_sem_take(&app_sem, K_SECONDS(1));
	// k_sem_take(&app_sem, K_NO_WAIT);
