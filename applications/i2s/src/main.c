...
// #define TIMEOUT_SECONDS K_FOREVER
...
static void i2s_codec_rx_sample(void) {
    ...
    codec_cfg.dai_cfg.i2s.options =
        I2S_OPT_BIT_CLK_CONT | I2S_OPT_BIT_CLK_MASTER | I2S_OPT_FRAME_CLK_MASTER | I2S_OPT_PINGPONG;
    ...
    while (!stop) {
        ...
        printf("[%s] rx_size %d line%d\n", __func__, rx_size, __LINE__);
...
int main(void) {
    i2s_codec_rx_sample();
    i2s_codec_tx_sample(); /* for rtl87x2g */
    // i2s_tx_sample(); /* for rtl8752h */
    return 0;
}
